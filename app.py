...
from database import init_db, save_report, get_latest_incident_location, get_user_plan, register_user
import requests
...
# -----------------------------
# Helper: Download Media with Auth
# -----------------------------
...
# -----------------------------
# Helper: Extract location / destinations
# -----------------------------
...
def extract_destinations(message):
    ...
    if " to " in message:
        ...
        return [d.strip().title() for d in parts[1].split(",")]
    return []
...
def reply_whatsapp():
    ...
    business_id = "default"  # can later assign per business
    register_user(user_number, business_id)  # ensure user exists
    ...
    if "alternative" in user_message_en.lower() or "route" in user_message_en.lower():
        latest_location = get_latest_incident_location(user_number, business_id)
        ...
        for dest in destinations:
            reply_en += f"\n📍 To {dest}: {google_route_link(latest_location, dest)}"
        resp.message(reply_en + "\n\n" + translate_to_urdu(reply_en))
        return Response(str(resp), mimetype="text/xml")

    # Accident report logic
    location = extract_location(user_message_en)
    save_report(user_number, "Accident", location, "Unknown", business_id)
    maps_link = f"https://www.google.com/maps/search/{location.replace(' ', '+')}"
    reply_en = f"🚨 Accident Report Saved:\nLocation: {location}\n📍 Map: {maps_link}"
    resp.message(reply_en + "\n\n" + translate_to_urdu(reply_en))
    return Response(str(resp), mimetype="text/xml")
